...
- `robo_configv5.inf` — **config actuelle** (celle appliquée par l'installateur)
- `robo_configv4.inf` — version précédente
- `OldConfig/` — anciennes versions archivées (`V1/`, `V3/`)

## Installation manuelle (sans l'outil)

```
.\Aslains_WoT_Modpack_Installer.exe /LOADINF=robo_configv5.inf
```
...
2. Télécharge `robo_configv5.inf` depuis GitHub (branche `main`)
3. Sauvegarde l'ancienne config, copie la nouvelle
4. Lance l'installateur Aslain avec `/LOADINF=robo_configv5.inf`
